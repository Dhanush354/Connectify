...
const CreatePost=()=>{
    ...
    const handleSubmit=async()=>{
        if(!images.length && !content) return toast.error('Post cannot be empty');
        setLoading(true);
    ...
    return(
        <div className="min-h-screen bg-gradient-to-b from-slate-50 to-white">
            <div className="max-w-6xl mx-auto p-6">
                ...
                {/*Form*/}
                <div className="max-w-xl bg-white p-4 sm:pb-3 rounded-xl shadow-md
                space-y-4">
                    {/*Header */}
                            ...
                            <div className="flex flex-wrap gap-2 mt-4">
                                {images.map((image,i)=>(
                                    <div key={i} className="relative group">
                                        <img src={URL.createObjectURL(image)} alt="" className="h-20
                                        rounded-md"/>
                                        <div onClick={()=> setImages(images.filter((_,index)=> index!==i))} className="absolute hidden group-hover:flex justify-center items-center top-0 right-0 bottom-0 left-0 
                                        bg-black/40 rounded-md cursor-poiter">
